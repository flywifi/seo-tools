...
6. Click **Add content** and add the knowledge, one of two ways:
   - **Simplest (one file):** upload the single combined pack
     `implementation/claude/project/creator-os-combined.md`. It contains all nine knowledge files in
     one, so there is nothing else to add. (It is generated by
     `python3 tools/build_freshness_bundle.py --apply`; do not hand-edit it.)
   - **Or the nine separate files** from `implementation/claude/project/knowledge/`:
   ...
   **How you get these files with no repo on your computer:** ask the maintainer to send them
   ...
   Drive hub screen), download them from the repository page (github.com/flywifi/seo-tools)
   under `implementation/claude/project/`, or use the GitHub-connector option above and skip
   files entirely. On a paid plan the plugin door also skips files: Customize > Plugins, add
   the marketplace link (`docs/UPDATING.md`).
...
You do not need to re-upload all 9 files for a small change. A GitHub-connected Project skips
re-uploads entirely: press "Sync now" instead.
