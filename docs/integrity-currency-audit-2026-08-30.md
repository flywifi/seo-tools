...
| F7 | MED | Dependency drift: 1 breaking (mcp 2.0.0, install-safe behind the `<2` pin), 3 minor (numpy, av, scenedetect), 8 previously had no baseline | closed P79-9 and P80: seven no-baseline entries validated by battery and stamped (P79-9); numpy 2.5.2, av 18.1.0, scenedetect 0.7.1 validated against the P26 goldens on Python 3.12 and mcp ported to the 2.x SDK (P80); faster-whisper moved to PyPI (P81, no-baseline) |
