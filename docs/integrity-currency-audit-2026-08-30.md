...
| F7 | MED | Dependency drift: 1 breaking (mcp 2.0.0, install-safe behind the `<2` pin), 3 minor (numpy, av, scenedetect), 8 previously had no baseline | P79-9: seven no-baseline entries validated by battery and stamped. P80: numpy 2.5.2, av 18.1.0, scenedetect 0.7.1 validated against the P26 goldens on Python 3.12 (the media recipe reproduced byte for byte, so this needed no Mac); mcp ported to the 2.x SDK |
