...
Fallback: No runtime or hosted seam -> lookups degrade to Class B (answer from the canonical-sources/construction dictionary and edition-status.json provided as knowledge); for calculations, reason over the shared/construction-engine.md "Calculators (first principles only)" formulas, show the arithmetic step by step, flag the result unverified, and name the tools/build_calc.py calc to run; never fabricate a quantity, span, or code figure.
