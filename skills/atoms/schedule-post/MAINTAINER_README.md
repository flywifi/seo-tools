...
## Purpose
This atom queues or schedules a single piece of finalized content to one social platform via the active publishing connector. It resolves the correct connector tier (Postiz MCP > Buffer MCP > direct platform API > manual fallback), runs FTC/AIGC compliance checks, presents a confirmation summary to the creator, and only then sends to the connector. Its job ends at the moment the post is queued — post-status handles follow-up status checks.
...
- Connector resolution order is fixed: Postiz MCP first, Buffer MCP second, per-platform direct API third, manual last.
...
## Known failure modes
- Missing media_url when direct API path is active: return `status: failed`. Postiz/Buffer do not require media_url.
...
1. Happy path: Postiz active, all fields present → `status: queued`, `publishing_tier: hosted_mcp`
...
When the connector returns ambiguous status (e.g., partial acceptance), the atom returns the conservative interpretation (failed) and surfaces the raw connector message in `error`. If two connectors are both active and disagree, Postiz takes precedence.

## Update checklist
1. Run `python3 tools/sync_check.py` — all 17 invariants must pass.
