...
## Purpose
Write one section of Alex Slason's media kit. Subscriber counts, engagement rates, and CPMs are null when real data is absent; benchmark ranges are labeled as industry benchmarks, not personal data.
...
- Voice is professional and brand-facing (published-to-audience mode, not planning-to-Alex).

## Known failure modes
- Presenting a benchmark engagement rate as Alex's personal rate.
- Omitting placeholders_to_fill when real subscriber data is unknown.
- Using planning-to-Alex voice in external-facing media kit copy.
